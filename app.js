...
function loadData() {
  try {
    ...
    parsed.cadeados = parsed.cadeados || [];
    return parsed;
  } catch (e) {
...
function cadeadosDe(alojamentoId) {
  return state.data.cadeados.filter((c) => c.alojamentoId === alojamentoId);
}
  ...
  openSheet({
    ...
    onMount: (root) => {
      ...
      root.querySelector('#save-cd').addEventListener('click', () => {
        ...
        if (editing) {
          editing.numero = numero; editing.alojamentoId = alojamentoId; editing.armarioId = armarioId;
        } else {
          state.data.cadeados.push({ id: uid('cd'), numero, alojamentoId, armarioId });
        }
        saveData();
        closeSheet();
