...
function loadData() {
  try {
    ...
    parsed.armarios = (parsed.armarios || []).map((a) => Object.assign({ patrimonio: '' }, a));
...
function generateArmarios(alojamentoId, qtd, inicioStr) {
  ...
  toCreate.forEach((numero) => {
    state.data.armarios.push({ id: uid('ar'), alojamentoId, numero, status: 'livre', usuario: '', patrimonio: '' });
  });
  return { ok: true };
}
      ...
      openConfirm(
        ...
        () => {
          state.data = {
            ...
            armarios: (parsed.armarios || []).map((a) => Object.assign({ patrimonio: '' }, a)),
