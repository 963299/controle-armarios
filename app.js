...
function loadData() {
  try {
    ...
    parsed.armarios = (parsed.armarios || []).map((a) => Object.assign({ patrimonio: '', idSolicitadaEm: null }, a));
...
function render() {
  ...
  switch (cur.view) {
    ...
    case 'solicitacoes': main.innerHTML = renderSolicitacoes(); break;
...
function renderDashboard() {
  ...
  if (alojamentos.length === 0) {
    ...
    list = `<div class="alojamento-list">` + alojamentos.slice().sort((a, b) => a.nome.localeCompare(b.nome, 'pt-BR', { sensitivity: 'base' })).map((al) => {
...
function renderAlojamentoDetail(alojamentoId) {
  ...
  if (all.length === 0) {
    ...
    grid = `<div class="locker-grid">` + visible.map((a) => {
      ...
      const isCadeadoLivre = a.status === 'livre' && hasLock;
      const colorClass = isCadeadoLivre ? 'st-cadeado' : `st-${meta.color}`;
      const label = isCadeadoLivre ? 'Com cadeado' : meta.label;
      return `
        <button class="locker-tile ${colorClass}" data-action="open-armario" data-id="${a.id}">
          ...
          ${a.status === 'ocupado' ? `<div class="locker-user">${escapeHtml(a.usuario)}</div>` : `<div class="locker-user" ${isCadeadoLivre ? '' : 'style="color:var(--text-faint)"'}>${label}</div>`}
        </button>
      `;
    }).join('') + `</div>`;
  }
...
function renderCadeados() {
    ...
    const rows = cadeados.map((c) => {
      ...
      return `
        <tr data-action="open-edit-cadeado" data-id="${c.id}" style="cursor:pointer;">
          ...
          <td>${arm ? `<span class="tag ${STATUS[arm.status].tag}">${STATUS[arm.status].label}</span>` : (c.alojamentoId ? `<span class="tag tag-avulso">Livre</span>` : '')}</td>
        </tr>
      `;
    }).join('');
    body = `
  ...
  const toolbar = state.data.alojamentos.length > 0 ? `
    ...
    <button class="btn btn-block" style="margin-bottom:16px;" data-action="export-cadeados-csv">📄 Exportar planilha de cadeados (.csv)</button>
  ` : '';
    ...
    onMount: (root) => {
      ...
      function refresh() {
        ...
        results.innerHTML = matches.map((c) => {
          ...
          const al = getAlojamento(c.alojamentoId);
          const arm = getArmario(c.armarioId);
          return `<div class="locate-result"><b>Cadeado ${escapeHtml(c.numero)}</b><br>${al ? escapeHtml(al.nome) : '—'} · ${arm ? `Armário ${escapeHtml(arm.numero)} <span class="tag ${STATUS[arm.status].tag}">${STATUS[arm.status].label}</span>` : `<span class="tag tag-avulso">Livre, sem armário específico</span>`}</div>`;
        }).join('');
      }
...
function openLocalizarArmarioVazioSheet() {
  const alojamentos = state.data.alojamentos.slice().sort((a, b) => a.nome.localeCompare(b.nome, 'pt-BR', { sensitivity: 'base' }));
  openSheet({
...
function openLocalizarPatrimonioSheet() {
  openSheet({
    title: 'Localizar por patrimônio',
    sub: 'Digite o número de patrimônio para ver a qual armário e alojamento ele pertence.',
    bodyHtml: `
      <div class="field">
        <label for="loc-pat-numero">Número de patrimônio</label>
        <input type="text" id="loc-pat-numero" placeholder="Ex.: PAT-00234">
      </div>
      <div id="loc-pat-results"></div>
    `,
    footerHtml: `<button class="btn btn-block" id="close-loc-pat">Fechar</button>`,
    onMount: (root) => {
      const input = root.querySelector('#loc-pat-numero');
      const results = root.querySelector('#loc-pat-results');
      function refresh() {
        const q = input.value.trim().toLowerCase();
        if (!q) { results.innerHTML = ''; return; }
        const matches = state.data.armarios.filter((a) => a.patrimonio && a.patrimonio.toLowerCase().includes(q));
        if (matches.length === 0) {
          results.innerHTML = `<p style="font-size:13px; color:var(--text-faint);">Nenhum armário encontrado com esse número de patrimônio.</p>`;
          return;
        }
        results.innerHTML = matches.map((a) => {
          const al = getAlojamento(a.alojamentoId);
          return `<div class="locate-result"><b>Patrimônio ${escapeHtml(a.patrimonio)}</b><br>${al ? escapeHtml(al.nome) : '—'} · Armário ${escapeHtml(a.numero)} <span class="tag ${STATUS[a.status].tag}">${STATUS[a.status].label}</span></div>`;
        }).join('');
      }
      input.addEventListener('input', refresh);
      input.focus();
      root.querySelector('#close-loc-pat').addEventListener('click', closeSheet);
    }
  });
}

function splitPastedCols(line) {
  if (line.includes('\t')) return line.split('\t').map((s) => s.trim());
  if (line.includes(';')) return line.split(';').map((s) => s.trim());
  return line.split(',').map((s) => s.trim());
}

function openImportarPatrimonioSheet() {
  const alojamentos = state.data.alojamentos.slice().sort((a, b) => a.nome.localeCompare(b.nome, 'pt-BR', { sensitivity: 'base' }));

  openSheet({
    title: 'Importar patrimônios',
    sub: 'Selecione as células no Excel, copie (Ctrl+C) e cole aqui embaixo.',
    bodyHtml: `
      <div class="field">
        <label for="pat-imp-alojamento">Se colar só 2 colunas (Armário, Patrimônio), de qual alojamento são?</label>
        <select id="pat-imp-alojamento">
          <option value="">— As linhas já trazem o alojamento (3 colunas) —</option>
          ${alojamentos.map((al) => `<option value="${al.id}">${escapeHtml(al.nome)}</option>`).join('')}
        </select>
      </div>
      <div class="field">
        <label for="pat-imp-text">Dados colados</label>
        <textarea id="pat-imp-text" rows="8" placeholder="Cole aqui: Armário [TAB] Patrimônio&#10;ou: Alojamento [TAB] Armário [TAB] Patrimônio"></textarea>
      </div>
      <div id="pat-imp-result"></div>
    `,
    footerHtml: `
      <button class="btn" id="cancel-pat-imp">Cancelar</button>
      <button class="btn btn-primary" id="run-pat-imp">Importar</button>
    `,
    onMount: (root) => {
      root.querySelector('#cancel-pat-imp').addEventListener('click', closeSheet);
      root.querySelector('#pat-imp-text').focus();
      root.querySelector('#run-pat-imp').addEventListener('click', () => {
        const fixedAlojamentoId = root.querySelector('#pat-imp-alojamento').value || null;
        const text = root.querySelector('#pat-imp-text').value;
        const lines = text.split(/\r?\n/).map((l) => l.trim()).filter(Boolean);
        if (lines.length === 0) { toast('Cole os dados antes de importar.'); return; }

        let updated = 0, notFound = 0, skipped = 0;
        const notFoundList = [];

        lines.forEach((line) => {
          const cols = splitPastedCols(line);
          if (/^(alojamento|armario|armário|patrim[oô]nio)$/i.test(cols[0])) { skipped++; return; }

          let alojamentoNome = null, armarioNum, patrimonio;
          if (cols.length >= 3) {
            [alojamentoNome, armarioNum, patrimonio] = cols;
          } else if (cols.length === 2) {
            [armarioNum, patrimonio] = cols;
          } else {
            skipped++; return;
          }
          if (!armarioNum) { skipped++; return; }

          let alojamentoId = fixedAlojamentoId;
          if (alojamentoNome) {
            const key = alojamentoNome.trim().toLowerCase();
            const al = state.data.alojamentos.find((a) => a.nome.trim().toLowerCase() === key);
            if (!al) { notFound++; notFoundList.push(`${alojamentoNome} / ${armarioNum}`); return; }
            alojamentoId = al.id;
          }
          if (!alojamentoId) { skipped++; return; }

          const arm = state.data.armarios.find((a) => a.alojamentoId === alojamentoId && a.numero === armarioNum.trim());
          if (!arm) { notFound++; notFoundList.push(`${armarioNum}`); return; }

          arm.patrimonio = (patrimonio || '').trim();
          updated++;
        });

        saveData();
        const resultBox = root.querySelector('#pat-imp-result');
        resultBox.innerHTML = `
          <div class="field-hint" style="margin-top:6px;">
            ✅ ${updated} atualizado(s) · ${notFound} não encontrado(s) · ${skipped} ignorada(s)
            ${notFoundList.length > 0 ? `<br><span style="color:var(--sem-id)">Não encontrados: ${notFoundList.slice(0, 15).map(escapeHtml).join(', ')}${notFoundList.length > 15 ? '…' : ''}</span>` : ''}
          </div>
        `;
        toast(`${updated} armário(s) atualizado(s).`);
        render();
      });
    }
  });
...
// Gera e baixa uma planilha (.csv, abre direto no Excel) com o controle de
// cadeados, no mesmo espírito da planilha "LISTA DOS CADEADOS" que o
// usuário já usava antes do app.
function exportCadeadosCSV() {
  if (state.data.cadeados.length === 0) {
    toast('Nenhum cadeado registrado ainda.');
    return;
  }
  const sorted = [...state.data.cadeados].sort((a, b) => {
    const na = parseInt(a.numero, 10);
    const nb = parseInt(b.numero, 10);
    if (!isNaN(na) && !isNaN(nb) && na !== nb) return na - nb;
    return String(a.numero).localeCompare(String(b.numero));
  });
  const rows = [['CADEADO', 'ALOJAMENTO', 'ARMARIO']];
  sorted.forEach((c) => {
    if (c.mode === 'local') {
      rows.push([c.numero, c.local || '', '']);
    } else {
      const al = getAlojamento(c.alojamentoId);
      const arm = getArmario(c.armarioId);
      rows.push([c.numero, al ? al.nome : '', arm ? arm.numero : '']);
    }
  });
  ...
  a.href = url;
  a.download = `controle-cadeados-${stamp}.csv`;
...
/* ---------------- Armários com identificação solicitada ---------------- */

function renderSolicitacoes() {
  const pendentes = state.data.armarios
    .filter((a) => a.idSolicitadaEm)
    .sort((a, b) => new Date(a.idSolicitadaEm) - new Date(b.idSolicitadaEm));

  if (pendentes.length === 0) {
    return `
      <div class="section-label">Identificação solicitada</div>
      <div class="empty-state">
        <svg class="empty-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M12 8v5l3 2"/></svg>
        <h3>Nenhuma solicitação pendente</h3>
        <p>Ao abrir um armário sem identificação, use "Marcar que já solicitei identificação" para ele aparecer aqui com a data.</p>
      </div>
    `;
  }
  ...
  const rows = pendentes.map((a) => {
    const al = getAlojamento(a.alojamentoId);
    return `
      <div class="person-card" data-action="open-armario" data-id="${a.id}" style="cursor:pointer;">
        <div class="person-name">
          <span class="tag tag-sem-id">${al ? escapeHtml(al.nome) : '—'} · nº ${escapeHtml(a.numero)}</span>
        </div>
        <div class="person-locker-row">
          <span>📌 Solicitado em ${formatDateTime(a.idSolicitadaEm)}</span>
        </div>
      </div>
    `;
  }).join('');
  ...
  return `
    <div class="section-label">Identificação solicitada <span>${pendentes.length}</span></div>
    <p style="font-size:11.5px; color:var(--text-faint); margin:-6px 0 12px;">Toque em um armário para atualizar o status ou remover a solicitação.</p>
    ${rows}
  `;
}
...
function renderAjustes() {
  ...
  return `
    ...
    <div class="section-label">Controle interno</div>
    <button class="btn btn-block" style="margin-bottom:10px;" data-action="open-localizar-patrimonio">Localizar por nº de patrimônio</button>
    <button class="btn btn-block" style="margin-bottom:10px;" data-action="open-importar-patrimonio">Importar patrimônios (colar do Excel)</button>
    <p style="font-size:11.5px; color:var(--text-faint); margin:-4px 0 10px; line-height:1.5;">
      O número de patrimônio fica dentro de cada armário. Não aparece na grade — é só para consulta interna.
    </p>
...
function generateArmarios(alojamentoId, qtd, inicioStr) {
  ...
  toCreate.forEach((numero) => {
    state.data.armarios.push({ id: uid('ar'), alojamentoId, numero, status: 'livre', usuario: '', patrimonio: '', idSolicitadaEm: null });
  });
  return { ok: true };
}
...
function openArmarioSheet(armarioId) {
  ...
  openSheet({
    ...
    bodyHtml: `
      ...
      <div id="id-request-block" class="${arm.status === 'sem_id' ? '' : 'hidden'}"></div>
      <div class="field">
        <label for="ar-cadeado">Nº do cadeado</label>
        <input type="text" id="ar-cadeado" placeholder="Ex.: 07" value="${lock ? escapeHtml(lock.numero) : ''}">
      </div>
      <div class="field">
        <label for="ar-patrimonio">Nº de patrimônio</label>
        <input type="text" id="ar-patrimonio" placeholder="Ex.: PAT-00234" value="${escapeHtml(arm.patrimonio || '')}">
      </div>
      ${arm.status === 'ocupado' ? `<button type="button" class="btn-ghost" id="share-wa" style="padding:0; font-size:12.5px; margin-top:8px;">📲 Enviar cadastro por WhatsApp</button>` : ''}
    `,
    footerHtml: `
    ...
    onMount: (root) => {
      ...
      const idBlock = root.querySelector('#id-request-block');

      function refreshIdBlock() {
        idBlock.innerHTML = arm.idSolicitadaEm ? `
          <div class="field-hint" style="margin-bottom:8px;">📌 Identificação solicitada em ${formatDateTime(arm.idSolicitadaEm)}.</div>
          <button type="button" class="btn-ghost" id="clear-id-request" style="padding:0; font-size:12.5px; margin-bottom:12px;">Remover solicitação</button>
        ` : `
          <button type="button" class="btn-ghost" id="mark-id-request" style="padding:0; font-size:12.5px; margin-bottom:12px;">📌 Marcar que já solicitei identificação</button>
        `;
        const markBtn = idBlock.querySelector('#mark-id-request');
        if (markBtn) markBtn.addEventListener('click', () => {
          arm.idSolicitadaEm = new Date().toISOString();
          saveData();
          refreshIdBlock();
          toast('Identificação marcada como solicitada.');
        });
        const clearBtn = idBlock.querySelector('#clear-id-request');
        if (clearBtn) clearBtn.addEventListener('click', () => {
          arm.idSolicitadaEm = null;
          saveData();
          refreshIdBlock();
      ...
      root.querySelectorAll('input[name="status"]').forEach((radio) => {
        radio.addEventListener('change', () => {
          ...
          idBlock.classList.toggle('hidden', radio.value !== 'sem_id');
        });
      });
      ...
      function saveCadeadoField() {
        const numero = root.querySelector('#ar-cadeado').value.trim();
        const existing = cadeadoDoArmario(arm.id);
        if (!numero) {
          if (existing) state.data.cadeados = state.data.cadeados.filter((c) => c.id !== existing.id);
          return;
        }
        if (existing) {
          existing.numero = numero;
        } else {
          state.data.cadeados.push({ id: uid('cd'), numero, mode: 'armario', alojamentoId: arm.alojamentoId, armarioId: arm.id, local: '' });
        }
      }
      ...
      root.querySelector('#save-ar').addEventListener('click', () => {
        ...
        arm.patrimonio = root.querySelector('#ar-patrimonio').value.trim();
        saveCadeadoField();
        if (status !== 'sem_id') arm.idSolicitadaEm = null;
        if (status === 'ocupado') {
...
function openAddCadeadoSheet(prefillId) {
  ...
  function armarioOptions(alojamentoId, selectedId) {
    const freeOption = `<option value="__free__" ${!selectedId ? 'selected' : ''}>— Livre (sem armário específico) —</option>`;
    const armOptions = armariosDe(alojamentoId).map((a) => {
      const sel = a.id === selectedId ? 'selected' : '';
      return `<option value="${a.id}" ${sel}>${escapeHtml(a.numero)} — ${STATUS[a.status].label}</option>`;
    }).join('');
    return freeOption + armOptions;
  }
    ...
    onMount: (root) => {
      ...
      root.querySelector('#save-cd').addEventListener('click', () => {
        ...
        if (mode === 'armario') {
          ...
          const armarioSel = arSelect.value;
          if (!alojamentoId) { toast('Cadastre um alojamento primeiro.'); return; }
          const armarioId = (!armarioSel || armarioSel === '__free__') ? null : armarioSel;
...
function mainClickHandler(e) {
  ...
  switch (action) {
    ...
    case 'open-localizar-armario-vazio':
      ...
      break;
    case 'open-localizar-patrimonio':
      openLocalizarPatrimonioSheet();
      break;
    case 'open-importar-patrimonio':
      openImportarPatrimonioSheet();
      break;
    case 'export-cadeados-csv':
      exportCadeadosCSV();
      break;
    case 'export-data':
      exportData();
      ...
      openConfirm(
        ...
        () => {
          state.data = {
            ...
            armarios: (parsed.armarios || []).map((a) => Object.assign({ patrimonio: '', idSolicitadaEm: null }, a)),
